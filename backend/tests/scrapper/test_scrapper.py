...
def test_fetch_random_recipes(mocker, n_fetched_recipes):
    ...
    mocker.patch.object(
        scrapper,
        "_fetch_metadata",
        return_value={"total_nb_recipes": total_nb_recipes, "nb_recipes_per_page": nb_recipes_per_page},
    )
    ...
    if n_fetched_recipes <= total_nb_recipes:  # Should work
        ...
        with pytest.raises(
            AssertionError, match=f"Cannot fetch {n_fetched_recipes} recipes, only {total_nb_recipes} available"
        ):
            scrapper.fetch_random_recipes(n_fetched_recipes)
